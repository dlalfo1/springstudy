...
public class PostController {
	...
	// 파라미터를 받은 3가지 방법
	// public String detail(HttpServletRequest request) { } : request 객체 이용
	// public String detail(@RequestParam("name") String name, @RequestParam("age") int age) { } : @RequestParam 애너테이션 이용
	// public String detail(Peson p) { } : 넘어온 파라미터를 필드변수로 하는 커맨드 객체 이용
	...
	public String detailMe(HttpServletRequest request,
		...
		redirectAttributes.addFlashAttribute("name", name);	// addAttribtu가 아님을 주의하세요!
	...
	public String listMe() { 	// Flash Attribute는 Redirect 경로까지 자동으로 전달되므로 별도의 경로가 필요 없다.
