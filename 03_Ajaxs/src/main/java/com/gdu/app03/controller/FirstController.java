...
public class FirstController {
	...
	@ResponseBody	// 메소드의 반환 값이 Jsp이름이 아니다. 응답하는 값(데이터)이다.
	...
	public Person ajax1(HttpServletRequest request,
						HttpServletResponse response) {	// first.jsp에서 보낸 파라미터 두개 name,age가 전달된다.
														// "Jackson 라이브러리"가 반환값 Person 객체를 자동으로 JSON 데이터로 변환한다.
		
		return firstService.execute1(request, response); // Person 객체를 만들어서 반환한다.
