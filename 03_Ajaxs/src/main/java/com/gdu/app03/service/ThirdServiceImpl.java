...
public class ThirdServiceImpl implements IThirdService {
	...
	@Override
	public ResponseEntity<Contact> execute1(Contact contact) { // JSON데이터를 객체로 받기
		
		// 이름 또는 전화번호가 공백인 경우 $.ajax의  error 처리
		if(contact.getName().isEmpty() || contact.getTel().isEmpty()) {
			return new ResponseEntity<Contact>(HttpStatus.BAD_REQUEST); // 에러쪽으론 응답코드만 보내준다.
		}
	...
	@Override
	public ResponseEntity<Map<String, String>> execute2(Map<String, String> map) {	// JSON데이터를 Map으로 받기
