...
public class FirstServiceImpl implements IFirstService {
	...
	public Person execute1(HttpServletRequest request, HttpServletResponse response) {
		...
		try {	
			...
			name = name.isEmpty() ? "홍길동" : name;	// 사용자가 입력한 name이 없으면 빈 문자열이 전달됩니다.
			
			String strAge = request.getParameter("age");
			strAge = strAge.isEmpty() ? "0" : strAge;	// 사용자가 입력한 age가 없으면 빈 문자열이 전달됩니다.
			...
			if(age < 0 || age > 100) {
				...
				return new Person(name, age); // try문의 응답 :  $.ajax의 success로 넘기는 값
			...
			try {
				...
				out.println(e.getMessage());	// catch문의 응답 :  $.ajax의 error로 넘기는 예외 메시지
				out.flush();
				out.close();
			} catch(Exception e2) {
				e2.printStackTrace();
