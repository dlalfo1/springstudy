...
<script>
    ...
    function fnCheckId (){
    	$('#id').on('keyup', function(){
    		...
    		// 입력한 아이디
    		let id = $(this).val(); // this : $('#id')
    		
    		// 정규식(5~40자, 소뭇자+숫자+하이픈(-)+밑줄(_) 사용 가능, 첫 글자는 소문자+숫자 사용 가능)
    		// [a-z0-9] : a~z or 0~9 사이의 한글자
    		let regId = /^[a-z0-9][a-z0-9-_]{4,39}$/;
    		
    		// 정규식 검사
    		verifyId = regId.test(id);
    		if(verifyId == false){
    			// span 태그에 글자 넣을 때 text 속성 사용.
    			$('#msgId').text('5~40자, 소뭇자+숫자+하이픈(-)+밑줄(_) 사용 가능, 첫 글자는 소문자+숫자 사용 가능');// span 태그에 글자 넣을 때 text 속성 사용.
    			return;  // 여기서 함수 실행을 종료한다. (이후에 나오는 ajax(중복체크) 실행을 막기 위해서)
    			         // 정규식 검사를 통과하지 못하면 중복체크할 필요가 없다.
    		}
    		...
    		$.ajax({
    			...
    			success: function(resData){ // resData = {"enableId": true} 또는 {"enableId": false}
    				                                        // Map map = new HashMap();
    				                                        // map.put("enableId", "true")
    				                                        // 이렇게 백에서 map을 만들어 반환해주면 jacson이 해석한다.
    ...
    function fnCheckEmail(){
      ...
      $('#btnGetCode').on('click', function(){
        ...
        new Promise(function(resolve, reject) {
        	...
        	reject(1);  // 정규식 실패 (catch블럭의 function 실행)
        	...
        	$.ajax({
        		success: function(){
    			  ...
    			  // 인증코드전송
    			  $.ajax({      -> 이렇게 then(성공) 안에서 또 ajax처리를 해줘야한다.
    				  
    			  })
    			
    		}).catch(function(number - 1 or 2가 전달된다.){  // reject() 함수 호출할 때 처리되는 함수(ajax 처리 error 했을 때)
    			  // number == 1 정규식 실패 메시지
    			  // number == 2 이메일 중복 체크 실패 메시지
