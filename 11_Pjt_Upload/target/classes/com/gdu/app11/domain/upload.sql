-- 1게시글 - 1첨부 - 1테이블
-- 1게시글(1) - 다중첨부(다) - 2테이블 => 일대다 관계 

-- 다중 첨부 게시판

-- 첨부 파일 정보 테이블 (다) - 일을 참조할 수 있는 외래키가 필요하다.
DROP TABLE ATTACH;
CREATE TABLE ATTACH (
    ...
    HAS_THUMBNAIL   NUMBER,                      -- 썸네일이 있으면 1(TRUE), 없으면 0(FALSE)
    UPLOAD_NO       NUMBER                       -- 게시글 FK(외래키를 정할 때 이름은 바뀌어도 되지만 타입은 바뀔 수 없다.)
);
...
CREATE TABLE UPLOAD (
    ...
    UPLOAD_TITLE VARCHAR(1000 BYTE) NOT NULL,   -- 제목
...
ALTER TABLE ATTACH
    ADD CONSTRAINT FK_ATTACH_UPLOAD
        FOREIGN KEY(UPLOAD_NO) REFERENCES UPLOAD(UPLOAD_NO) 
             ON DELETE CASCADE; -- 게시글이 지워지면 첨부파일의 정보도 삭제되어야 한다.
