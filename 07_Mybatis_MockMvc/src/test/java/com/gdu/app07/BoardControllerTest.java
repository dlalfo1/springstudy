...
@WebAppConfiguration // 통합테스트 진행시 WebApplicationContext 사용하기 위해서 필요한 애너테이션
public class BoardControllerTest {
	...
	/*
		Mock 테스트
		1. 가상 MVC 테스트이다.
		2. Controller를 테스트할 수 있는 통합테스트이다.
	...
	*/
	...
	// WebApplicationContext : 내장객체로 @Autowired 애너테이션으로 Bean을 주입해주기만 하면 된다.
	// 테스트 진행시 기본으로 필요한 놈.
	@Autowired
	private WebApplicationContext webApplicationContext; 
	
	// LOGGER
	private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(BoardControllerTest.class);
	...
	@Before
	public void setUp() throws Exception {	// MockMvc 객체 생성하기.
		...
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
		        .post("/board/add.do")					// @PostMapping("/board/add.do")
		        	...
		        	.param("writer", "테스트작성자"))	// 파라미터
  					...
  					// 결과를 Model에 담아서 보내는 경우(SELECT)엔 getModelAndView, FlashAttribute(삽입,수정,삭제) 를 사용한 경우엔 getFlashMap
  							.toString());
	...
	public void a2수정테스트() throws Exception {
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
		        .post("/board/modify.do")					// @PostMapping("/board/add.do")
		        	...
		        	.param("boardNo", "1"))					// 파라미터 (boardNo 파라미터가 넘어올 때 문자열로 넘어온다.
  						.andReturn()						// 수정결과
  						.getFlashMap() 						// FlashAttribute에 저장된 결과 확인
  					// 결과를 Model에 담아서 보내는 경우(SELECT)엔 getModelAndView, FlashAttribute(삽입,수정,삭제) 를 사용한 경우엔 getFlashMap
  							.toString());
