...
public class DBConfig {
	...
	// JdbcTemplate Bean (Connection, PrepareStatement, ResultSet을 이용해서 동작하는 스프링 클래스)
	@Bean
	...
	// 아래 부분은 AOP를 이용해 트랜잭션 처리를 하기 위한 Bean
	...
	public TransactionInterceptor transactionInterceptor() {
		...
		ruleBasedTransactionAttribute.setName("*");	// 모든 이름에서 동작한다.
	...
	/*
		AOP 관련 핵심 용어 3가지 (조인포인트에서 포인트컷을 선정 후 어드바이스를 적용한다.)
		1. 조인포인트(JoinPoint) : AOP를 적용시킬 수 있는 메소드 전체      - 목록, 상세, 삽입, 수정, 삭제
		2. 포인트컷(PintCut)     : 조인포인트 중에서 AOP를 동작시킬 메소드 - 삽입, 수정, 삭제
	...
	*/
	...
	public Advisor advisor() {
		...
		*/
		...
		return new DefaultPointcutAdvisor(pointCut, transactionInterceptor());	//  pointCut으로 등록된 메소드 transactionInterceptor()를 동작시킨다.
