...
public class BlogServiceImpl implements BlogService {
	...
	public void loadBlogList(HttpServletRequest request, Model model) {
		...
		// 모든 리스트를 가져올 때 페이지네이션을 적용하고 싶다면 첫번째 작업은 page 파라미터를 받는 것이다.
		...
		// page, blogCount, recordPerPage 값만 알면 페이지네이션을 구현할 수 있다.
		...
		// <1 2 3 4 5> 이 중 어떤걸 눌러도 getPagination(path) 여기 path로 이동한다.
	...
	public void addBlog(HttpServletRequest request, HttpServletResponse response) {
	  ...
	  if(elements != null) {
		  for(Element element : elements) {
			  ...
			  summernoteImageDTO.setBlogNo(blogDTO.getBlogNo()); // blogDTO에 이미 들어가 있는 blogNo를 꺼내온다.
	  ...
	  try {
		  ...
		  out.println("<script>");
		  if(addResult == 1) {
			  out.print("alert('블로그가 작성되었습니다.');");
			  out.print("location.href='" + request.getContextPath() + "/blog/list.do';");
		  } else {
			  out.print("alert('블로그가 작성이 실패했습니다.');");
	...
	public Map<String, Object> imageUpload(MultipartHttpServletRequest multipartRequest) {
		...
		*/
		...
		// 첨부된 파일의 타입은 MultipartFile 이다.
		MultipartFile multipartFile = multipartRequest.getFile("file"); // 이 이름이 ajax요청시 보낸 file의 이름임.
		...
		// HDD에 저장된 파일의 확인을 위한 mapping값을 반환(servlet-context.xml 참고할 것)
		Map<String, Object> map = new HashMap<String, Object>();
